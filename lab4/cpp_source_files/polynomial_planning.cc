...
// Generates a circle trajectory with radius 1 with numWaypoints
std::vector<p4::NodeEqualityBound> circleTraj(int numWaypoints){
  std::vector<p4::NodeEqualityBound> ans = {
    // First node with no initial acceleration or velocity
  ...
  for(int i = 1; i <= numWaypoints; i++){
    ans.push_back(p4::NodeEqualityBound(0,i,0,cos(i*2*M_PI/numWaypoints)));
    ans.push_back(p4::NodeEqualityBound(1,i,0,sin(i*2*M_PI/numWaypoints)));
  }
  return ans;
}
...
void NumWaypointExperiments() {
  ...
  for(int i = 0; i <= numWaypoints; i++){
    times.push_back(i*10.0/numWaypoints);
  }

  // The parameter order for p4::NodeEqualityBound is:
  // (dimension_index, node_idx, derivative_idx, value)
  const std::vector<p4::NodeEqualityBound> node_equality_bounds = circleTraj(numWaypoints);
